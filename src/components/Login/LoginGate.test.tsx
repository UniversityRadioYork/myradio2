...
it("dispatches a check action", () => {
    const { getDispatchedActions } = renderWithRedux(<LoginGate>null</LoginGate>, {})
...
it("renders a login iframe when there's no login", () => {
    const { getByTestId, dispatch } = renderWithRedux(<LoginGate>null</LoginGate>, {})
...
it("renders children when there's a signed in user", () => {
    const { getByTestId, queryByTestId, dispatch } = renderWithRedux(<LoginGate><div data-testid="success" /></LoginGate>, {})

    dispatch(state.actions.setCurrentUser({
        fname: "",
        sname: "",
        public_email: "",
        bio: "",
